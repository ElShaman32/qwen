          ...
          switch (tipo) {
            ...
            case 'arqueo_parcial':
              ...
              final aperturaId = aperturaUuid != null
                  ...
                  : null;
              if (aperturaId == null) {
                _logger.w('⚠️ Arqueo parcial sin apertura local: $uuid');
                continue;
              }
              await db
                  .into(db.arqueoParcial)
                  .insert(ArqueoParcialCompanion.insert(
                    uuid: uuid,
                    aperturaId: aperturaId,
                    usuarioId: (data['usuarioId'] as String?) ?? '',
                    usuarioNombre: (data['usuarioNombre'] as String?) ?? '',
                    montoEsperadoBs:
                        (data['montoEsperadoBs'] as num?)?.toDouble() ?? 0,
                    montoContadoBs:
                        (data['montoContadoBs'] as num?)?.toDouble() ?? 0,
                    diferenciaBs:
                        (data['diferenciaBs'] as num?)?.toDouble() ?? 0,
                    nota: Value(data['nota'] as String?),
                    fecha: (data['fecha'] as num?)?.toInt() ?? 0,
...
// ───────────────────────────────────────────────────────────────────────
  ...
  Future<bool> _existeCajaPorUuid(AppDatabase db, String uuid) async {
    ...
    final ar = await (db.select(db.arqueoParcial)
          ..where((t) => t.uuid.equals(uuid)))
        .getSingleOrNull();
    return ar != null;
  }
  ...
  Future<void> _completarCaja(ClientFirebase clientFb) async {
    try {
      ...
      final arqueos = await (db.select(db.arqueoParcial)).get();
      for (final a in arqueos) {
        if (remotos.contains(a.uuid)) continue;
        await clientFb.firestore.collection('caja').doc(a.uuid).set({
          'tipo': 'arqueo_parcial',
          'aperturaUuid': uuidPorId[a.aperturaId],
          'usuarioId': a.usuarioId,
          'usuarioNombre': a.usuarioNombre,
          'montoEsperadoBs': a.montoEsperadoBs,
          'montoContadoBs': a.montoContadoBs,
          'diferenciaBs': a.diferenciaBs,
          'nota': a.nota,
          'fecha': a.fecha,
        }, SetOptions(merge: true));
      }
