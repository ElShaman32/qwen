...
class AppConfigState {
  ...
  const AppConfigState({
    ...
    this.timestampUltimaVerificacionMaestro = 0,
  });
  ...
  /// El usuario ya aceptó los términos legales en el onboarding
  bool get yaAceptoTerminos => acceptedLegal;
...
class AppConfigNotifier extends Notifier<AppConfigState> {
  ...
  Future<void> loadFromCache() async {
    try {
      ...
      if (row != null) {
        _logger
            .i('📦 Configuración cargada: acceptedLegal=${row.acceptedLegal}');
        state = AppConfigState(
          ...
          acceptedLegal: row.acceptedLegal,
          timestampUltimaVerificacionMaestro:
              row.timestampUltimaVerificacionMaestro,
        );
  ...
  Future<void> activarModoDemo(int timestamp) async {
    try {
      ...
      state = state.copyWith(
        ...
        acceptedLegal: state.acceptedLegal, // ← PRESERVAR acceptedLegal
      );
  ...
  /// Guarda que el usuario aceptó los términos legales.
  /// Persiste en Drift para que sobreviva a reinicios de la app.
  Future<void> aceptarTerminos() async {
    try {
      _logger.i('🔐 Iniciando guardar aceptación de términos...');
      final db = ref.read(databaseProvider);
      final existente =
          await (db.select(db.configuracionLocal)..limit(1)).getSingleOrNull();

      _logger
          .i('🔐 Configuración existente: ${existente != null ? "SÍ" : "NO"}');

      if (existente == null) {
        _logger.i('🔐 Creando nueva configuración con acceptedLegal=true');
        await db.into(db.configuracionLocal).insert(
              ConfiguracionLocalCompanion.insert(
                acceptedLegal: const Value(true),
              ),
            );
      } else {
        _logger.i(
            '🔐 Actualizando configuración existente con acceptedLegal=true');
        await (db.update(db.configuracionLocal)
              ..where((t) => t.id.equals(existente.id)))
            .write(
      ...
      state = state.copyWith(acceptedLegal: true);
      _logger.i(
          '✅ Términos legales aceptados y persistidos. Estado actualizado: acceptedLegal=${state.acceptedLegal}');
    } catch (e, stack) {
      _logger.e('❌ Error guardando aceptación de términos',
  ...
  /// Source of truth dinámico: solo descarga si hay internet.
  /// Sin internet: usa cache de Drift (ya cargado en loadFromCache).
  Future<void> syncFromRemote() async {
    final clientFb = ref.read(clientFirebaseProvider);
    final online = ref.read(connectivityProvider).valueOrNull ?? false;

    if (!clientFb.isInitialized || !online) {
      _logger.w(
          '⚠️ Sin internet o ClientFirebase no inicializado, usando cache local');
    ...
    try {
      ...
      state = AppConfigState(
        ...
        isLoading: false,
        isOfflineMode: false,
        acceptedLegal: state.acceptedLegal,
        isDemoMode: state.isDemoMode,
        demoStartTimestamp: state.demoStartTimestamp,
        timestampUltimaVerificacionMaestro:
            state.timestampUltimaVerificacionMaestro,
      );
  ...
  /// Actualiza el timestamp de verificación contra el Maestro.
  /// Llamar DESPUÉS de verificar exitosamente la suscripción.
  Future<void> actualizarVerificacionMaestro(int timestamp) async {
    try {
      final db = ref.read(databaseProvider);
      final existente =
          await (db.select(db.configuracionLocal)..limit(1)).getSingleOrNull();
      ...
      if (existente != null) {
        await (db.update(db.configuracionLocal)
            ...
            .write(
          ConfiguracionLocalCompanion(
            timestampUltimaVerificacionMaestro: Value(timestamp),
          ),
        );
      }

      state = state.copyWith(timestampUltimaVerificacionMaestro: timestamp);
      _logger.i('🔐 Verificación del Maestro actualizada: $timestamp');
    } catch (e) {
      _logger.e('❌ Error actualizando verificación del Maestro: $e');
    }
  }
}
