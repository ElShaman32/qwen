...
/// Lee metodos_pago con source of truth dinámico:
/// - Con internet: Firestore (fuente de verdad)
/// - Sin internet: defaults locales (fallback)
final metodosPagoProvider = FutureProvider<List<MetodoPago>>((ref) async {
  ...
  // Sin internet o sin ClientFirebase: usar defaults
  ...
  try {
    ...
    // Error de red o permisos: fallback a defaults
    return MetodoPago.defaultsActivos;
  }
});

/// Todos los métodos (para Configuración), orden estable por id.
/// Source of truth dinámico: Firestore con internet, lista vacía sin internet.
final metodosPagoAdminProvider = FutureProvider<List<MetodoPago>>((ref) async {
  final clientFb = ref.watch(clientFirebaseProvider);
  final online = ref.watch(connectivityProvider).valueOrNull ?? false;

  // Sin internet o sin ClientFirebase: no puede mostrar configuración
  if (!clientFb.isInitialized || !online) {
    return const [];
  }
  ...
  try {
    ...
    Logger(printer: PrettyPrinter(methodCount: 0))
        .e('Error leyendo métodos de pago: $e');
    return const [];
  }
});
