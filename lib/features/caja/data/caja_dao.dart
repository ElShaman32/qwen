...
class CajaDao {
  ...
  Future<AperturaCajaData> abrir({
    ...
    // ⬇️ AGREGA ESTO ⬇️
    // Movimiento crítico: registrar apertura
    await registrarMovimiento(
      _db,
      usuarioId: usuarioId,
      usuarioNombre: usuarioNombre,
      accion: AccionesAuditoria.cajaApertura,
      detalles: 'Inicial: Bs ${montoInicialBs.toStringAsFixed(2)} / '
          '\$${montoInicialUsd.toStringAsFixed(2)}'
          '${(novedad != null && novedad.isNotEmpty) ? ', novedad: $novedad' : ''}',
    );
    // ⬆️ FIN ⬆️
  ...
  Future<void> retirar({
    ...
    // Necesitamos el UUID de la apertura para el payload de Firestore
    ...
    // Outbox — AHORA SÍ envía el UUID correcto
    ...
    await registrarMovimiento(
      ...
      usuarioId: usuarioId,
      usuarioNombre: usuarioNombre,
      accion: AccionesAuditoria.retiroCaja,
      detalles: 'Bs ${montoBs.toStringAsFixed(2)}, motivo: $motivo',
    );
  }
  ...
  Future<CierreCajaData> cerrar({
    ...
    final id = await _db.transaction(() async {
      ...
      // Movimiento crítico: registrar cierre
      await registrarMovimiento(
        _db,
        usuarioId: usuarioId,
        usuarioNombre: usuarioNombre,
        accion: AccionesAuditoria.cierreCaja,
        detalles: 'Esperado: Bs ${montoEsperadoBs.toStringAsFixed(2)}, '
            'contado: Bs ${montoRealBs.toStringAsFixed(2)}, '
            'diferencia: Bs ${diferencia.toStringAsFixed(2)}',
      );
  ...
  /// Registra un arqueo parcial (conteo intermedio).
  Future<void> arquearParcial({
    required int aperturaId,
    required double montoEsperadoBs,
    ...
    final ahora = DateTime.now().millisecondsSinceEpoch;
    final diferencia = _red2(montoContadoBs - montoEsperadoBs);
    final arqueoUuid = const Uuid().v4();
    ...
    await _db.into(_db.arqueoParcial).insert(ArqueoParcialCompanion.insert(
          uuid: arqueoUuid,
          aperturaId: aperturaId,
          usuarioId: usuarioId,
          usuarioNombre: usuarioNombre,
    ...
    // Outbox (usamos aperturaUuid para mantener consistencia con Firestore)
    await encolarSync(_db, coleccion: 'caja', docId: arqueoUuid, payload: {
      'tipo': 'arqueo_parcial',
      'aperturaUuid': apertura.uuid,
      'usuarioId': usuarioId,
      'usuarioNombre': usuarioNombre,
    ...
    // Movimiento crítico (Auditoría)
    await registrarMovimiento(
      _db,
      usuarioId: usuarioId,
      usuarioNombre: usuarioNombre,
      accion:
          'arqueo_parcial', // O AccionesAuditoria.arqueoParcial si lo tienes tipado
      detalles: 'Esperado: Bs ${montoEsperadoBs.toStringAsFixed(2)}, '
          'contado: Bs ${montoContadoBs.toStringAsFixed(2)}, '
          'diferencia: Bs ${diferencia.toStringAsFixed(2)}',
    );
  }

  /// Lista de arqueos parciales de una apertura específica.
  Future<List<ArqueoParcialData>> arqueosDe(int aperturaId) {
    return (_db.select(_db.arqueoParcial)
          ..where((t) => t.aperturaId.equals(aperturaId))
          ..orderBy([
            (t) => OrderingTerm(expression: t.fecha, mode: OrderingMode.desc)
          ]))
