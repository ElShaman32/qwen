...
import '../../ventas/data/caja_requerida_provider.dart';
...
class CajaScreen extends ConsumerWidget {
      ...
      children: [
          ...
          Card(
            child: Padding(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('Arqueos parciales del turno',
                      style: theme.textTheme.titleSmall),
                  ...data.arqueos.map((a) => ListTile(
                        contentPadding: EdgeInsets.zero,
                        dense: true,
                        leading: Icon(Icons.search,
                            color: a.diferenciaBs == 0
                                ? const Color(0xFF4CAF50)
                                : theme.colorScheme.error,
                            size: 20),
                        title: Text(
                          'Por ${a.usuarioNombre} · ${Formato.fecha(DateTime.fromMillisecondsSinceEpoch(a.fecha))}',
                          style: theme.textTheme.bodySmall,
                        ),
                        subtitle: Text(
                          'Esperado: ${Formato.bs(a.montoEsperadoBs)} | Contado: ${Formato.bs(a.montoContadoBs)}',
                          style: theme.textTheme.bodySmall,
                        ),
                        trailing: Text(
                          a.diferenciaBs == 0
        ...
        Row(
          children: [
            ...
            Expanded(
              child: OutlinedButton.icon(
                onPressed: () => _arqueoParcialDialog(context, ref, data),
        ...
        actions: [
          ...
          FilledButton(
            onPressed: () async {
              ...
              ref.invalidate(debeExigirCajaProvider);
  ...
  Future<void> _arqueoParcialDialog(
      BuildContext context, WidgetRef ref, CajaStateData data) async {
    final apertura = data.apertura!;
    final resumen = data.resumen!;
    ...
    await showDialog(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setState) {
          final contado = double.tryParse(
              contadoController.text.trim().replaceAll(',', '.'));
          final diferencia =
              contado != null ? _red2(contado - resumen.esperadoBs) : null;

          return AlertDialog(
            title: const Text('Arqueo parcial'),
            content: SingleChildScrollView(
              child: Column(
                mainAxisSize: MainAxisSize.min,
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('Esperado en cajón: ${Formato.bs(resumen.esperadoBs)}',
                  ...
                  TextField(
                    ...
                    decoration: const InputDecoration(
                      labelText: 'Nota (opcional)',
                      hintText: 'Ej: Faltan billetes de 20',
                    ),
                  ),
                ],
              ),
            ...
            actions: [
              ...
              FilledButton(
                onPressed: contado == null
                    ? null
                    : () async {
                        final user = ref.read(currentUserProvider).value;
                        await ref.read(cajaDaoProvider).arquearParcial(
                              aperturaId: apertura.id,
                              montoEsperadoBs: resumen.esperadoBs,
                              montoContadoBs: _red2(contado),
                              nota: notaController.text.trim().isEmpty
                                  ? null
                                  : notaController.text.trim(),
                              usuarioId: user?.uid ?? '',
                ...
                child: const Text('¡Listo! Registrar'),
              ),
            ],
          );
        },
