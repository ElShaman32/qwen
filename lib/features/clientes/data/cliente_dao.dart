...
class ClienteDao {
  ...
  /// Clientes con fiado vencido (saldo > 0 Y sin actividad hace > 7 días).
  Future<List<ClienteData>> conFiadoVencido() {
    final hace7Dias = DateTime.now().subtract(const Duration(days: 7));
    final hace7DiasEpoch = hace7Dias.millisecondsSinceEpoch ~/ 1000;

    return (_db.select(_db.cliente)
          ..where((t) =>
              t.activo.equals(true) &
              t.saldoPendienteUsd.isBiggerThanValue(0) &
              t.fechaActualizacion.isSmallerThanValue(hace7DiasEpoch))
          ..orderBy([
            (t) => OrderingTerm(
                expression: t.fechaActualizacion, mode: OrderingMode.asc)
          ]))
        .get();
  }
}
