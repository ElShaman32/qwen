...
final routerProvider = Provider<GoRouter>((ref) {
  return GoRouter(
    ...
    redirect: (context, state) async {
      ...
      // 3. MODO DEMO VENCIDO → bloquear TODO excepto Activación
      if (appConfig.isDemoMode && appConfig.demoVencido) {
        if (!isActivacion) {
          return AppRoutes.activacion;
        }
        return null; // Permitir estar en Activación
      }

      // 4. MODO DEMO ACTIVO → permitir acceso sin sesión Firebase
      if (appConfig.isDemoMode && !appConfig.demoVencido) {
        // El Splash ya mandó a home (no interceptar splash)
        if (isSplash) return null;
        // Permitir ir a Activación (el usuario puede querer salir del demo)
        if (isActivacion) return null;
        // Si intenta ir a login/registro/suspensión desde fuera, mandar al home
        if (isLogin || isRegistro || isSuspension) {
