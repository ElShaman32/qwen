...
class _SplashScreenState extends ConsumerState<SplashScreen> {
  ...
  void _navegarSiguiente() {
    ...
    // Si llegó aquí, el kill switch NO está activo
    final haySesion =
        clientFb.isInitialized && clientFb.auth.currentUser != null;

    // 🔵 PRIORIDAD 2: Hay sesión activa → dashboard
    if (haySesion) {
      context.go(AppRoutes.home);
      return;
    }

    // 🔵 PRIORIDAD 3: No hay sesión + config vacía → activación (primera vez)
    if (config.appNombre.isEmpty) {
      context.go(AppRoutes.activacion);
      return;
    }

    // 🔵 PRIORIDAD 4: Hay config pero ClientFirebase no se pudo restaurar
    //    (ej: activación con versión vieja que no guardó credenciales).
    //    → ir a Activación para re-inicializar y guardar credenciales.
    if (!clientFb.isInitialized) {
      context.go(AppRoutes.activacion);
      return;
    }

    // 🔵 PRIORIDAD 5: Caso normal uso posterior → login
